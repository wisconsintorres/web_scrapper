...
function scrapeItems (URL){
    ...
    request(URL, function (err, response, body) {
        ...
        let press_link = [];
        const results = [];
        const URLElements = $('td').find('time');
        const URLElementsA = $('td').find('a');
        ...
        for( let i =0; i < press_date.length; i++ ){
            ...
            results[i] = {
                ...
                month: month,
                year: year,
                org: 'United States Senate Committee on Armed Services'
            }
        }
...
scrapeItems('https://www.armed-services.senate.gov/press');